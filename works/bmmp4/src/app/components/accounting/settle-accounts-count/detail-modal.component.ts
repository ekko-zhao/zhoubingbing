...
export class DetailModalComponent {
    ...
    public modalInit(transmit, postForm) {
        ...
        this.postForm['remDate'] = this.datePipe.transform(transmit['remDate'], env['format']);
        this.postForm['terminalNo'] = transmit['termId'];
    ...
    // 设置 存储缓存的键
    public formStorageKey: string = 'settleDetailModalCountForm';
    public tableLocalKey: string = 'settleDetailModalTable';

    // 配置表格
    public table = {
        th: [
            { key: 'endRemDate', text: '结算日期', require: true },
            { key: 'merchantNo', text: '商户号', require: true },
            { key: 'merchantName', text: '商户名称', require: true },
            { key: 'terminalNo', text: '终端号', require: true },
            { key: 'shopName', text: '网点名称', require: true },
            { key: 'transferAccountDate', text: '调账日期', require: true },
    ...
    public search(e?, page?: number, size: number = env['itemsPerPage']) {
        ...
        this.http.post('api/v1/transferAccount/queryTransferAccountDetail', this.postForm).subscribe(
            response => {
