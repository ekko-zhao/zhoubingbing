...
export class AccountInfoComponent {
    ...
    // 设置 存储缓存的键
    public formStorageKey: string = 'accountInfoForm';
    public tableLocalKey: string = 'accountInfoTable';
    ...
    public table = {
        th: [
            ...
            { key: 'shopName', text: '网点名称', require: false },
            { key: 'transferAccountDate', text: '调账日期', require: false },
            { key: 'transferAccountTime', text: '调账时间', require: true },
            { key: 'transferAccountType', text: '调账类型', require: true },
            { key: 'transferAccountAmount', text: '调账金额', require: true },
            { key: 'settleStatus', text: '结算状态', require: true },
            { key: 'settleDate', text: '结算日期', require: false },
            { key: 'remark', text: '备注', require: true }
        ],
        storage: []
    };
    ...
    constructor(
        ...
        // 下拉列表
        myService.getSelectList(this, 'selectList', 'TRANSFER_ACCOUNT_TYPE');
    }
    ...
    // 查询时间
    public timeStart = 'startRegDate';
    public timeEnd = 'endRegDate';
    ...
    public search(e?, page: number = 1, size: number = env['itemsPerPage']) {
        ...
        window['loading']['open']();
        this.http.post('api/v1/transferAccount/queryTransferAccount', this.postForm).subscribe(
            response => {
    ...
    public phCount = {};
    public searchCount() {
        this.http.post('api/v1/transferAccount/queryTransferAccountSum', this.postForm).subscribe(
            response => {
                if (response['retCode'] === '000000') {
    ...
    public reset() {
        ...
        // 初始化表单
        this.form = {};
        this.form['transferAccountType'] = null;
        let date = new Date();
