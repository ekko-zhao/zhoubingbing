...
@Component({
    templateUrl: './count-direct.html',
    providers: [
        HttpService,
        DatePipe
    ]
})
export class CountDirectComponent {
    // 表单
    public form = {};
    public postForm = {};
    public regex = regex;
    ...
    // 设置 存储缓存的键
    public formStorageKey: string = 'tradeCountDirectForm';
    public tableLocalKey: string = 'tradeCountDirectTable';
    ...
    public search(e?, page?: number, size: number = env['itemsPerPage']) {
        ...
        window['loading']['open']();
        this.http.post('api/v1/directTransaction/queryTransSummary', this.postForm).subscribe(
            response => {
    ...
    public searchCount() {
        ...
        data['queryType'] = 'sum';
        this.http.post('api/v1/directTransaction/queryTransSummary', data).subscribe(
            response => {
                if (response['retCode'] === '000000') {
