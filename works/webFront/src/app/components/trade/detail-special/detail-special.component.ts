...
@Component({
    templateUrl: './detail-special.html',
    providers: [
        HttpService,
        DatePipe
    ]
})
export class DetailSpecialComponent implements OnInit {
    // 表单
    public form = {};
    public postForm = {};
    public regex = regex;
    ...
    // 设置 存储缓存的键
    public formStorageKey: string = 'tradeDetailSpecialForm';
    public tableLocalKey: string = 'tradeDetailSpecialTable';

    // 配置表格
    public table = {
        th: [
            { key: 'clearDate', text: '清算日期', require: true },
            { key: 'merchantCodeOld', text: '商户号', require: true },
            { key: 'mercName', text: '商户名称', require: false },
            { key: 'terminalNoOld', text: '终端号', require: true },
            ...
            { key: 'acceptNo', text: '受理流水', require: true },
            { key: 'srefNo', text: '参考号', require: true },
            { key: 'authCode', text: '授权码', require: true }
    ...
    public search(e?, page?: number, size: number = env['itemsPerPage']) {
        ...
        if (e && e !== true) {
            ...
            /* 如果查询条件有时间 需要特殊处理 */
            let f = this.myService.formatTime(this, true);
            if (!f) return;
        ...
        window['loading']['open']();
        this.http.post('api/v1/specialTransaction/queryTransDetail', this.postForm).subscribe(
            response => {
    ...
    public searchCount() {
        ...
        data['queryType'] = 'aggs';
        this.http.post('api/v1/specialTransaction/queryTransSummary', data).subscribe(
            response => {
                if (response['retCode'] === '000000') {
