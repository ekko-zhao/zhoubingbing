...
@Component({
    templateUrl: './count-special.html',
    providers: [
        HttpService,
        DatePipe
    ]
})
export class CountSpecialComponent {
    // 表单
    public form = {};
    public postForm = {};
    public regex = regex;
    ...
    // 设置 存储缓存的键
    public formStorageKey: string = 'tradeCountSpecialForm';
    public tableLocalKey: string = 'tradeCountSpecialTable';
    ...
    public captionType = {
        ...
        termId: [
            ...
            { key: 'tranTypeName', text: '交易类型', require: false },
            { key: 'count', text: '交易笔数', require: true },
            { key: 'tranAmount', text: '交易金额', require: true },
            { key: 'fee', text: '手续费', require: true },
            { key: 'liqAmount', text: '应结算金额', require: false }
    ...
    public search(e?, page?: number, size: number = env['itemsPerPage']) {
        ...
        window['loading']['open']();
        this.http.post('api/v1/specialTransaction/queryTransSummary', this.postForm).subscribe(
            response => {
